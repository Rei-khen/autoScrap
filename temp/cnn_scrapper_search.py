...
CONFIG = {
    ...
    #   "relevansi" → urutan relevansi (default website, tanpa parameter result_type)
    #   "terbaru"   → urutan terbaru (result_type=latest)
    "sort": "terbaru",

    # Filter relevansi: True = query harus ada di JUDUL artikel
    # Berguna untuk menyaring artikel yang hanya menyebut query di isi (tidak relevan)
    # Sama seperti perilaku website CNN Indonesia
    "filter_judul": False,
...
# ──────────────────────────────────────────────────────────────
# Filter relevansi — query harus ada di judul
# ──────────────────────────────────────────────────────────────

def passes_query_filter(judul: str, query: str) -> bool:
    """
    Cek apakah kata kunci query ada di judul artikel.
    Matching case-insensitive, mendukung multi-kata.
    Contoh: query="iran as" → judul harus mengandung "iran" DAN "as"
    """
    if not query:
        return True
    judul_lower = judul.lower()
    # Semua kata dalam query harus ada di judul
    words = query.lower().split()
    return all(w in judul_lower for w in words)
...
def main():
    ...
    for search_cfg in cfg["searches"]:
        ...
        # Tentukan result_type berdasarkan setting sort
        sort_cfg = cfg.get("sort", "relevansi").strip().lower()
        result_type = "latest" if sort_cfg == "terbaru" else ""

        # Halaman 1 → ambil total
        api_params_base = {"query": query}
        if result_type:
            api_params_base["result_type"] = result_type
        resp = get_json({**api_params_base, "page": 1})
        if not resp:
            continue
        ...
        for page in range(1, total_pages + 1):
            ...
            if page in pages_data:
                ...
                resp = get_json({**api_params_base, "page": page})
                if not resp:
                    break
                items = resp.get("data", [])
            ...
            for item in items:
                ...
                # Filter relevansi: query harus ada di judul
                if cfg.get("filter_judul", True) and not passes_query_filter(judul, query):
                    log.debug(f"    Skip (query tidak di judul): {judul[:60]}")
                    continue
