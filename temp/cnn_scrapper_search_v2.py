"""
CNN Indonesia Scraper - Mode SEARCH (Final)
============================================
Menggunakan endpoint API internal: GET /api/search
yang mengembalikan JSON langsung.

Catatan hasil inspect API:
  - Pagination  : parameter ?page=N  (10 item/halaman)
  - Total item  : field 'total' di response
  - Filter kanal: TIDAK didukung server → difilter client-side via 'intidjnkanal'
  - Filter tgl  : TIDAK didukung server → difilter client-side via 'dtnewsdate'
  - URL artikel : field 'url' sudah lengkap
  - Tanggal     : field 'dtnewsdate' (format: 2026/04/19 22:19:23)

Penggunaan:
  python cnn_scraper_search.py

Output: cnn_search_result.json
"""

import requests
from bs4 import BeautifulSoup
import json
import math
import time
import re
import logging
from datetime import datetime
from urllib.parse import urlencode

# ─────────────────────────────────────────────────────────────
# CONFIG
# ─────────────────────────────────────────────────────────────
CONFIG = {
    # Daftar query pencarian. Key yang tersedia:
    #   query     : kata kunci (wajib)
    #   kanal     : nasional | internasional | ekonomi | olahraga |
    #               teknologi | otomotif | edukasi | hiburan | gaya-hidup | tv
    #               (hapus/kosongkan = semua kanal)
    #   fromdate  : DD/MM/YYYY  (opsional)
    #   todate    : DD/MM/YYYY  (opsional)
    "searches": [
        {
            "query":    "iran",
            "kanal":    "nasional",
            "fromdate": "19/04/2026",
            "todate":   "19/04/2026",
        },
        # Contoh lain:
        # {"query": "ekonomi"},
        # {"query": "bola", "fromdate": "17/04/2026", "todate": "18/04/2026"},
        # {"query": "teknologi", "kanal": "teknologi"},
    ],

    # Batas maksimal halaman per query (None = semua halaman)
    "max_halaman": None,

    # Jeda antar request (detik)
    "delay_halaman": 1.0,
    "delay_artikel": 0.8,

    # True  → buka halaman artikel untuk ambil isi bersih (tanpa HTML tag/iklan)
    # False → ambil isi dari field 'strisi' di API (lebih cepat, ada HTML tag)
    "ambil_detail_artikel": True,

    # File output
    "output_file": "./result/cnn_search_result.json",
}
# ─────────────────────────────────────────────────────────────

BASE_URL   = "https://www.cnnindonesia.com"
API_URL    = f"{BASE_URL}/api/search"
MEDIA_NAME = "CNN Indonesia"
PER_PAGE   = 10  # item per halaman (fixed dari server)

# Mapping nama kanal → daftar intidjnkanal yang valid
# intidjnkanal adalah "parent kanal" di setiap artikel API
KANAL_ID_MAP = {
    "nasional":      [3, 12],
    "internasional": [6, 4],
    "ekonomi":       [5, 2],
    "olahraga":      [7],
    "teknologi":     [8],
    "otomotif":      [577],
    "edukasi":       [559],
    "hiburan":       [9],
    "gaya-hidup":    [10],
    "tv":            [398, 11],
}

logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [%(levelname)s] %(message)s",
    datefmt="%H:%M:%S",
)
log = logging.getLogger(__name__)

HEADERS = {
    "User-Agent": (
        "Mozilla/5.0 (Windows NT 10.0; Win64; x64) "
        "AppleWebKit/537.36 (KHTML, like Gecko) "
        "Chrome/124.0.0.0 Safari/537.36"
    ),
    "Accept": "application/json, */*",
    "Accept-Language": "id-ID,id;q=0.9",
    "Referer": f"{BASE_URL}/",
}

SESSION = requests.Session()
SESSION.headers.update(HEADERS)


# ──────────────────────────────────────────────────────────────
# HTTP helpers
# ──────────────────────────────────────────────────────────────

def get_json(params: dict, retries: int = 3) -> dict | None:
    for attempt in range(1, retries + 1):
        try:
            r = SESSION.get(API_URL, params=params, timeout=15)
            r.raise_for_status()
            return r.json()
        except (requests.RequestException, ValueError) as e:
            log.warning(f"[{attempt}/{retries}] API error: {e}")
            if attempt < retries:
                time.sleep(3)
    log.error(f"Gagal hit API: params={params}")
    return None


def get_soup(url: str, retries: int = 3) -> BeautifulSoup | None:
    for attempt in range(1, retries + 1):
        try:
            r = SESSION.get(url, timeout=15)
            r.raise_for_status()
            return BeautifulSoup(r.text, "html.parser")
        except requests.RequestException as e:
            log.warning(f"[{attempt}/{retries}] HTTP error {url}: {e}")
            if attempt < retries:
                time.sleep(3)
    log.error(f"Gagal fetch: {url}")
    return None


# ──────────────────────────────────────────────────────────────
# Filter client-side (tanggal & kanal)
# ──────────────────────────────────────────────────────────────

def parse_dtnewsdate(val: str) -> datetime | None:
    """Parse field dtnewsdate → datetime. Format: '2026/04/19 22:19:23'"""
    try:
        return datetime.strptime(val.strip(), "%Y/%m/%d %H:%M:%S")
    except (ValueError, AttributeError):
        return None


def parse_user_date(val: str) -> datetime | None:
    """Parse tanggal dari user → datetime. Format: 'DD/MM/YYYY'"""
    try:
        return datetime.strptime(val.strip(), "%d/%m/%Y")
    except (ValueError, AttributeError):
        return None


def item_passes_filter(item: dict, kanal: str, fromdate: str, todate: str) -> bool:
    """
    Cek apakah artikel lolos filter kanal dan rentang tanggal.
    Filter dilakukan client-side karena server API tidak mendukungnya.
    """
    # Filter kanal via intidjnkanal (parent kanal ID)
    if kanal:
        allowed_ids = KANAL_ID_MAP.get(kanal.lower(), [])
        if allowed_ids:
            jn_kanal = int(item.get("intidjnkanal") or 0)
            if jn_kanal not in allowed_ids:
                return False

    # Filter tanggal via dtnewsdate
    if fromdate or todate:
        dt = parse_dtnewsdate(item.get("dtnewsdate", ""))
        if dt is None:
            return False
        if fromdate:
            fr = parse_user_date(fromdate)
            if fr and dt.date() < fr.date():
                return False
        if todate:
            to = parse_user_date(todate)
            if to and dt.date() > to.date():
                return False

    return True


def is_past_date_range(item: dict, fromdate: str) -> bool:
    """
    Cek apakah artikel sudah melewati batas bawah tanggal.
    Digunakan untuk early-stop: API mengembalikan artikel dari terbaru ke terlama,
    jadi jika dtnewsdate sudah lebih lama dari fromdate, halaman berikutnya
    tidak perlu di-fetch lagi.
    """
    if not fromdate:
        return False
    dt = parse_dtnewsdate(item.get("dtnewsdate", ""))
    fr = parse_user_date(fromdate)
    if dt and fr:
        return dt.date() < fr.date()
    return False


# ──────────────────────────────────────────────────────────────
# Parse detail artikel (isi bersih dari halaman)
# ──────────────────────────────────────────────────────────────

def clean_html(html_str: str) -> str:
    """Konversi HTML strisi → plain text bersih sebagai fallback."""
    if not html_str:
        return ""
    soup = BeautifulSoup(html_str, "html.parser")
    for junk in soup.select("script, style, iframe, div[class*='ads']"):
        junk.decompose()
    paragraphs = [p.get_text(strip=True) for p in soup.find_all("p") if p.get_text(strip=True)]
    return "\n\n".join(paragraphs) if paragraphs else soup.get_text(separator="\n", strip=True)


def scrape_article_detail(url: str) -> tuple[str, str]:
    """
    Buka halaman artikel → return (isi_bersih, tanggal_publikasi).
    """
    soup = get_soup(url)
    if not soup:
        return "", ""

    # Isi artikel
    isi = ""
    content_div = soup.select_one("div.detail-text")
    if content_div:
        for junk in content_div.select(
            "div.paradetail, div[class*='ads'], table.linksisip, "
            "script, style, center, div[id*='gpt'], iframe, "
            "div[data-type], a.embed, .inbetween_ads"
        ):
            junk.decompose()
        paragraphs = [
            p.get_text(strip=True)
            for p in content_div.find_all("p")
            if p.get_text(strip=True)
        ]
        isi = "\n\n".join(paragraphs)

    # Tanggal publikasi
    tgl = ""
    el = soup.select_one("div.text-cnn_grey")
    if el:
        tgl = el.get_text(strip=True)
    else:
        el = soup.find("time")
        if el:
            tgl = el.get("datetime") or el.get_text(strip=True)

    return isi, tgl


# ──────────────────────────────────────────────────────────────
# Main
# ──────────────────────────────────────────────────────────────

def main():
    cfg       = CONFIG
    results   = []
    seen_urls = set()

    for search_cfg in cfg["searches"]:
        query    = search_cfg.get("query", "").strip()
        kanal    = search_cfg.get("kanal", "").strip()
        fromdate = search_cfg.get("fromdate", "").strip()
        todate   = search_cfg.get("todate", "").strip()

        if not query:
            log.warning("Query kosong, dilewati.")
            continue

        label = f"query='{query}'"
        if kanal:    label += f"  kanal={kanal}"
        if fromdate: label += f"  dari={fromdate}"
        if todate:   label += f"  s/d={todate}"
        log.info(f"═══ Search: {label} ═══")

        # Halaman pertama → ambil total
        resp = get_json({"query": query, "page": 1})
        if not resp:
            continue

        total_items = int(resp.get("total") or 0)
        total_pages = math.ceil(total_items / PER_PAGE) if total_items else 1
        if cfg["max_halaman"]:
            total_pages = min(total_pages, cfg["max_halaman"])

        log.info(f"Total item di server: {total_items}  →  {total_pages} halaman")

        pages_data = {1: resp.get("data", [])}  # simpan page 1 agar tidak fetch ulang

        stop_early = False
        artikel_lolos = 0

        for page in range(1, total_pages + 1):
            if stop_early:
                break

            if page in pages_data:
                items = pages_data[page]
            else:
                time.sleep(cfg["delay_halaman"])
                resp = get_json({"query": query, "page": page})
                if not resp:
                    break
                items = resp.get("data", [])

            if not items:
                log.info(f"  Halaman {page} kosong, berhenti.")
                break

            log.info(f"  Halaman {page}/{total_pages}: {len(items)} item dari server")

            for item in items:
                # Early stop: artikel sudah lebih lama dari fromdate
                if fromdate and is_past_date_range(item, fromdate):
                    log.info(f"  Artikel sudah lebih lama dari {fromdate}, early stop.")
                    stop_early = True
                    break

                # Filter kanal & tanggal client-side
                if not item_passes_filter(item, kanal, fromdate, todate):
                    continue

                url   = item.get("url", "")
                judul = item.get("strjudul", "")
                tgl_api = item.get("dtnewsdate", "")

                if not url and not judul:
                    continue

                # Deduplicate
                key = url or judul
                if key in seen_urls:
                    continue
                seen_urls.add(key)

                artikel_lolos += 1
                log.info(f"  ✓ [{artikel_lolos}] {judul[:70]}...")

                if cfg["ambil_detail_artikel"] and url:
                    time.sleep(cfg["delay_artikel"])
                    isi, tgl = scrape_article_detail(url)
                    if not isi:
                        isi = clean_html(item.get("strisi", ""))
                    if not tgl:
                        tgl = tgl_api
                else:
                    isi = clean_html(item.get("strisi", ""))
                    tgl = tgl_api

                results.append({
                    "url":              url,
                    "judul":            judul,
                    "isi":              isi,
                    "tanggalPublikasi": tgl,
                    "media":            MEDIA_NAME,
                })

        log.info(f"  Artikel lolos filter: {artikel_lolos}")

    # Simpan JSON
    with open(cfg["output_file"], "w", encoding="utf-8") as f:
        json.dump(results, f, ensure_ascii=False, indent=2)

    log.info(f"\n✓ Selesai! {len(results)} artikel → '{cfg['output_file']}'")
    return results


if __name__ == "__main__":
    main()